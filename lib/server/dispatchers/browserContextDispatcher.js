...
class BrowserContextDispatcher extends _dispatcher.Dispatcher {
  constructor(parentScope, context) {
    ...
    this.addObjectListener(_browserContext.BrowserContext.Events.OnAction, data => {
      let action = data.action.name;
      let selector = data.action.selector;
      let text = data.action.text;
      let url = data.action.url;
      this._dispatchEvent(_browserContext.BrowserContext.Events.OnAction, {
        action,
        selector,
        text,
        url
