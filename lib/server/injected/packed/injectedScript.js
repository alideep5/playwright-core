...
function generateAriaTree(rootElement) {
  const toAriaNode = (element) => {
    const role = getAriaRole(element);
    if (!role)
      return null;
    const name = role ? getElementAccessibleName(element, false) || void 0 : void 0;
  ...
  const visit = (ariaNode, node) => {
    ...
    if (visible) {
      ...
      if (isHiddenContainer || !(childAriaNode == null ? void 0 : childAriaNode.isLeaf))
        processChildNodes((childAriaNode == null ? void 0 : childAriaNode.ariaNode) || ariaNode, element);
    } else {
      processChildNodes(ariaNode, element);
    }
  };
  function processChildNodes(ariaNode, element) {
    for (let child = element.firstChild; child; child = child.nextSibling)
...
function renderedAriaTree(rootElement) {
...
function normalizeStringChildren(rootA11yNode) {
  ...
  const visit = (ariaNode) => {
    ...
    flushChildren(buffer, normalizedChildren);
    ariaNode.children = normalizedChildren.length ? normalizedChildren : void 0;
  };
  visit(rootA11yNode);
}
var hiddenContainerRoles = /* @__PURE__ */ new Set(["none", "presentation"]);
...
function nodeMatches(root, template) {
  const results = [];
  const visit = (node) => {
...
function renderAriaTree(ariaNode) {
  const lines = [];
  const visit = (ariaNode2, indent) => {
    var _a, _b, _c;
    if (typeof ariaNode2 === "string") {
      lines.push(indent + "- text: " + escapeYamlString(ariaNode2));
      return;
    }
    let line = `${indent}- ${ariaNode2.role}`;
    if (ariaNode2.name)
      line += ` ${escapeWithQuotes(ariaNode2.name, '"')}`;
    const noChild = !ariaNode2.name && !((_a = ariaNode2.children) == null ? void 0 : _a.length);
    const oneChild = !ariaNode2.name && ((_b = ariaNode2.children) == null ? void 0 : _b.length) === 1 && typeof ariaNode2.children[0] === "string";
    if (noChild || oneChild) {
      if (oneChild)
        line += ": " + escapeYamlString((_c = ariaNode2.children) == null ? void 0 : _c[0]);
      lines.push(line);
      return;
    }
    lines.push(line + (ariaNode2.children ? ":" : ""));
...
function escapeYamlString(str) {
  ...
  const needQuotes = (
    ...
    /^\s|\s$/.test(str) || // Contains control characters
    /[\x00-\x1f]/.test(str) || // Contains special YAML characters that could cause parsing issues
    /[\[\]{}&*!,|>%@`]/.test(str) || // Contains a colon followed by a space (could be interpreted as a key-value pair)
    /:\s/.test(str) || // Is a YAML boolean or null value
    /^(?:y|Y|yes|Yes|YES|n|N|no|No|NO|true|True|TRUE|false|False|FALSE|null|Null|NULL|~)$/.test(str) || // Could be interpreted as a number
    /^[-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?$/.test(str) || // Contains a newline character
    /\n/.test(str) || // Starts with a special character
    /^[\-?:,>|%@"`]/.test(str)
  );
  if (needQuotes) {
